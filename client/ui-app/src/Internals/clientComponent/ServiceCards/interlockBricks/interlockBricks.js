...
const InterlockBricksCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("interlock bricks"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading Interlock Bricks...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Interlock Bricks Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don't have any businesses matching "Interlock Bricks"
