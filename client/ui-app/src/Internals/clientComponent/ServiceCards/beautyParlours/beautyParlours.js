...
const BeautyParloursCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("beauty parlour"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading beauty parlours...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Beauty Parlours Found Yet 😔</p>
