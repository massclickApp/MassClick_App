...
const SpaAndMassageCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("spa"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading spa & massage services...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Spa / Massage Services Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don’t have any businesses matching "Spa" or "Massage" at the moment.
                </p>
