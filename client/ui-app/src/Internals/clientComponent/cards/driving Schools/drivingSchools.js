...
const DrivingSchoolsCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("driving school"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading driving schools...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Driving Schools Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don't have any driving school businesses in our data right now.
                </p>
