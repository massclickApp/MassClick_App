...
const CourierServiceCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("courier service"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading courier services...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Courier Services Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don't have any courier service businesses in our data right now.
                </p>
