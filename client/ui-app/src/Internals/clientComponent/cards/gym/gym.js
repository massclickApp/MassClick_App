...
const GymCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("gym"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading gyms...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Gyms Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don’t have any gym or fitness center businesses in our data right now.
                </p>
