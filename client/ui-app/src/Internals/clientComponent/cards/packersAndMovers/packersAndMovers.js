...
const PackersAndMoversCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("packers and movers"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading packers & movers...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Packers & Movers Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don’t have any packers and movers listed in our database right now.
                </p>
