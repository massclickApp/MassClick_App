...
const EventOrganisersCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("event organiser"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading event organisers...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Event Organisers Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don’t have any event organiser businesses in our data right now.
                </p>
