...
const PetShopsCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("pet shop"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading pet shops...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Pet Shops Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don’t have any pet shops, pet stores or pet service providers in our data right now.
                </p>
