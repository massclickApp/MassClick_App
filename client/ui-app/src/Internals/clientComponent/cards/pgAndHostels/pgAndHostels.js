...
const PgAndHostelsCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("hostel"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading PGs & Hostels...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No PG / Hostels Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don’t have any PGs, hostels or boarding services in our data right now.
                </p>
