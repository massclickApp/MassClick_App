...
const EducationCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("education"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading education services...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Education Services Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don’t have any education-related businesses in our data right now.
                </p>
