...
const HomeDecorCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("home decor"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading home decor services...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Home Decor Services Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don’t have any home decor or interior decoration businesses in our data right now.
                </p>
