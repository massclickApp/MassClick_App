...
const EstateAgentCards = () => {
    ...
    useEffect(() => {
        dispatch(getBusinessByCategory("estate agent"));
    }, [dispatch]);
    ...
    if (loading) {
        return <p className="loading-text">Loading estate agents...</p>;
    }

    if (!loading && categoryBusinessList.length === 0) {
        return (
            <div className="no-results-container">
                <p className="no-results-title">No Estate Agents Found Yet 😔</p>
                <p className="no-results-suggestion">
                    It looks like we don’t have any real estate agent businesses in our data right now.
                </p>
